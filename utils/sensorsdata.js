...
sa.store = {
  ...
  getIsFirstDay: function(){
    return (typeof sa.store._state === 'object' && typeof sa.store._state.first_visit_day_time === 'number' && sa.store._state.first_visit_day_time > (new Date()).getTime());
  },
  getProps: function () {
...
sa.login = function (id, p) {
  var firstId = sa.store.getFirstId();
  var distinctId = sa.store.getDistinctId();
  if (id !== distinctId) {
    if (firstId) {
      sa.trackSignup(id, '$SignUp', p);
    } else {
...
if(sa.para.autoTrack !== false){
  ...
  // 公众号来源转换格式
  function transformUtm(para) {
    if (para && !para.query.utm_source) {
      if (para.referrerInfo && para.referrerInfo.appId) {
        para.query.utm_source = 'gongzhonghao';
        para.query.utm_campaign = para.referrerInfo.appId;
        para.query.utm_term = 'guanlian';
      } else if (['1005', '1006', '1019', '1042'].indexOf(para.scene) >= 0) {
        para.query.utm_source = 'organic';
        para.query.utm_medium = para.scene;
      }
    }
  }
  // 添加自定义参数
  function registerCustomParam(para) {
    var type = para.query.type;
    var share_type = type == 1 ? '凑步数' : type == 2 ? '普通分享' : type == 3 ? 
      '凑好友' : type == 4 ? '红包' : type == 5 ? '商品拉新' : type == 6 ? 
        '商品拉新图片' : type == 7 ? '凑好友图片' : type == 8 ? '步数日志' : type == 9 ? 
          '签到邀请' : type == 10 ? '商品拉新公众号' : type;
    sa.registerApp({
      is_first_day: sa.store.getIsFirstDay(),
      last_share_content_id: para.query.share_id,
      last_share_together_prdid: para.query.productId,
      last_share_model_name: para.query.share_button,
      last_share_pahway: para.scene,
      last_share_type: share_type,
      last_from_route: para.query.route_from
    });
  }
